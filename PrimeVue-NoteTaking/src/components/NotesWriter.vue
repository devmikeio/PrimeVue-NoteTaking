...
function filterNotes() {
  currentNotes.value = notes.value.filter((note) =>
...
function searchNotes() {
  currentNotes.value = [
    notes.value.find((note) => note.title === selectedNote.value.title),
  ];
}
</script>
  ...
  <div>
    <Panel header="Notes Writer">
          ...
          <div class="flex">
            ...
            <div class="p-inputgroup">
              ...
              <AutoComplete
                ...
                :suggestions="foundNotes"
                @complete="searchNote($event)"
                @item-select="searchNotes"
                @blur="selectedNote = ''"
              />
            </div>
          </div>
      ...
      <div class="flex flex-wrap justify-content-around gap-3">
        <div class="text-xl" v-if="!notes.length">
          No notes have been created yet. Hit the <b>New</b> button to create
          one.
        </div>
        <Card
          class="w-3 bg-bluegray-900 shadow-4"
        ...
        >
          ...
          <template #footer>
            ...
            <Button
              ...
              :icon="note.fav ? 'pi pi-heart-fill' : 'pi pi-heart'"
              @click="note.fav = !note.fav"
            />
            <Button
